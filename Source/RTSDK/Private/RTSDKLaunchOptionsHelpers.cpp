...
FString FRTSDKLaunchOptionsHelpers::GetLaunchOptionsFromPIEMatchSetup(const TArray<FRTSDKPIETeamSetupInfo>& inPIETeams)
{
    ...
    {
        PIELaunchOptions += FString::Printf(TEXT("?Team%i=|Name=%s"), t, *inPIETeams[t].DisplayName.ToString());
        ...
        {
            PIELaunchOptions += FString::Printf(TEXT("?Force%i=|Name=%s|Team=%i"), forceidx, *inPIETeams[t].PIEForces[f].DisplayName.ToString(), t);
            ...
            for (int32 c = 0; c < inPIETeams[t].PIEForces[f].PIECommanders.Num(); c++)
            {
                PIELaunchOptions = inPIETeams[t].PIEForces[f].PIECommanders[c].bIsBot ?
                    PIELaunchOptions + FString::Printf(TEXT("?Commander%i=|Name=%s|Force=%i|IsPlayer=false"), commanderidx, *inPIETeams[t].PIEForces[f].PIECommanders[c].DisplayName.ToString(), f) :
                    PIELaunchOptions + FString::Printf(TEXT("?Commander%i=|Name=%s|Force=%i|IsPlayer=true|PlayerID=%i"), commanderidx, *inPIETeams[t].PIEForces[f].PIECommanders[c].DisplayName.ToString(), f, humanidx);
